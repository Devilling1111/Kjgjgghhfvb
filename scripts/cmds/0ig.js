...
module.exports.onStart = async({message,api,event,args,client,Users,Threads,__GLOBAL,Currencies}) => {
...
;
  var mim = badoll[Math.floor(Math.random() * badollength)];
  var link = [
...
"https://i.postimg.cc/5tsJvjjV/received-583147518.jpg",
...
"https://i.postimg.cc/4NcXMJ26/received-819496329472643.jpg",
	 ...
	 var callback = () => api.sendMessage({body:`「 ${mim} 」`,attachment: fs.createReadStream(__dirname + "/badol.jpg")}, event.threadID, () => fs.unlinkSync(__dirname + "/badol.jpg"));	
      return request(encodeURI(link[Math.floor(Math.random() * link.length)])).pipe(fs.createWriteStream(__dirname+"/badol.jpg")).on("close",() => callback());
