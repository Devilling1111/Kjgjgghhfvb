...
;
  var know = nazrul[Math.floor(Math.random() * nazrul.length)];
  var link = [
...
"https://i.postimg.cc/4NcXMJ26/received-819496329472643.jpg",
	 ...
	 var callback = () => api.sendMessage({body:`「 ${know} 」`,attachment: fs.createReadStream(__dirname + "/nazrul.jpg")}, event.threadID, () => fs.unlinkSync(__dirname + "/nazrul.jpg"));	
      return request(encodeURI(link[Math.floor(Math.random() * link.length)])).pipe(fs.createWriteStream(__dirname+"/nazrul.jpg")).on("close",() => callback());
