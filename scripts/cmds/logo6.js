...
module.exports.onLoad = async() => {
    ...
    const filePath = resolve(__dirname, 'cache/canvas', 'frame6.jpeg');
    if (!existsSync(dirMaterial + "canvas")) mkdirSync(dirMaterial, { recursive: true });
    if (!existsSync(filePath)) await downloadFile("https://i.imgur.com/GAWwQR5.jpeg", filePath);
};

async function makeImage({ one, two }) {
    const __root = path.resolve(__dirname, "cache", "canvas");

    let batgiam_img = await jimp.read(__root + "/frame6.jpeg");
